...
import { MapPin, Beef, Leaf, Star } from 'lucide-react'
import Link from 'next/link'

export function RancherProfileComponent() {
  const rancher = {
    name: "Green Pastures Ranch",
    location: "Montana",
    image: "/placeholder.svg?height=200&width=200",
    description: "Family-owned ranch specializing in grass-fed, organic beef. Our cattle roam freely on lush Montana pastures.",
    practices: ["Grass-fed", "Organic", "Hormone-free"],
    rating: 4.8,
    products: [
      { id: 1, name: "Half Cow", price: 1500, image: "/placeholder.svg?height=100&width=100" },
      { id: 2, name: "Quarter Cow", price: 800, image: "/placeholder.svg?height=100&width=100" },
      { id: 3, name: "Assorted Cuts Box", price: 200, image: "/placeholder.svg?height=100&width=100" },
    ]
  }
          ...
          <div className="md:col-span-1">
            <Card className="bg-deer">
              <CardHeader>
                ...
                <CardTitle className="text-2xl text-center text-bistre mt-4">{rancher.name}</CardTitle>
                <CardDescription className="text-center text-sepia">
                  <MapPin className="inline-block mr-1" /> {rancher.location}
                </CardDescription>
              </CardHeader>
              <CardContent>
                <p className="text-bistre mb-4">{rancher.description}</p>
              ...
              <CardFooter className="justify-center">
                <div className="flex items-center text-sepia">
                  ...
                  <span>{rancher.rating} / 5</span>
                </div>
              </CardFooter>
            </Card>
          </div>
          <div className="md:col-span-2">
            <h2 className="text-3xl font-bold mb-6 text-bistre">Available Products</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {rancher.products.map((product) => (
                <Card key={product.id} className="bg-deer">
                  <CardHeader>
                    <div className="w-full h-48 bg-deepChampagne rounded-t-lg overflow-hidden">
                      <img src={product.image} alt={product.name} className="w-full h-full object-cover" />
                    </div>
                  </CardHeader>
                  <CardContent>
                    <CardTitle className="text-xl text-bistre">{product.name}</CardTitle>
                    <CardDescription className="text-sepia">${product.price}</CardDescription>
                  </CardContent>
                  <CardFooter>
